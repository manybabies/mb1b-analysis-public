---
title: "MB1B Data Reading and Merge"
...
This Rmd is the first preprocessing file for the primary ManyBabies 1 Bilingual (IDS Preference) dataset. The goal is to get everything into a single datafile that can be used for subsequent analyses. 
...
* `04_merge_with_monolinguals` pulls monolingual data from MB1 public repo and merges with bilinguals
* `05_confirmatory_analysis` is the set of confirmatory analyses that were preregisted (see [here](https://osf.io/grqau/))
* `06_exploratory_analysis` contains other, non-preregistered analyses for mb1
...
#devtools::install_github("langcog/langcog") #Necessary package not on CRAN
...
As with participants, define some basic descriptive statistics of the td_raw data that should not be altered by data cleaning, merge or exclusion criteria *unless we really mean to do so.* 

```{r}
n_trial_rows = nrow(td_raw)
n_unique_labs_from_trials <- td_raw %>%
  summarize(labs = n_distinct(lab))
unique_participants_by_lab_from_trials <- td_raw %>%
  group_by(lab) %>%
  summarize(participants = n_distinct(subid))
```

Directly after read-in, there are `r n_trial_rows` rows in td_raw, representing data from `r n_unique_labs_from_trials$labs` labs. 
...
# Commenting out transformations not needed here 
...
# two subids lack 0 padding
td$subid[td$lab == "babylab_kingswood" & td$subid == "mb904"] <- "mb0904"
td$subid[td$lab == "babylab_kingswood" & td$subid == "mb53"] <- "mb0053"

#Adjust lab names to match across MB1 and MB1B
pd$lab[pd$lab == "paris descartes_manybabies 1_bilinguals"] <- "babylabparisdescartes1"
td$lab[td$lab == "paris descartes_manybabies 1_bilinguals"] <- "babylabparisdescartes1"

pd$lab[pd$lab == "ispdevlab-mcgill"] <- "isplabmcgill"
td$lab[td$lab == "ispdevlab-mcgill"] <- "isplabmcgill"
...
Changing empty spaces to 0s for lang3/lang3_exposure/lang4/lang4_exposure.
```{r}

pd$lang3[is.na(pd$lang3)] <- 0
pd$lang3_exposure[is.na(pd$lang3_exposure)] <- 0
pd$lang4[is.na(pd$lang4)] <- 0
pd$lang4_exposure[is.na(pd$lang4_exposure)] <- 0

```

Sanitizing case of lang1/2/3/4 columns.
```{r}
pd$lang1 <- tolower(pd$lang1)
pd$lang2 <- tolower(pd$lang2)
pd$lang3 <- tolower(pd$lang3)
pd$lang4 <- tolower(pd$lang4)
...
Because subid sometimes repeats across labs, merge lab and subid to create a unique subid for each paticipant.

```{r}
pd <- pd %>% 
  unite(subid, lab, subid, remove = FALSE)
...
* These labs have more participants in trial than in participants - particularly confusing! As of 8/28, the following issues remain. 
...
* Finally, these are labs are missing a data source altogether (or are bad lab-id values). As of 12/15, no labs are here, suggesting we have all the datafiles! 
...
```{r}
validate_that(n_unique_labs_from_participants == n_unique_labs_from_trials)
validate_that(all(lab_checker$participant_concord))
```

# Language exposure checking


```{r}
#Check that language exposure adds up to 100 (+/- 1%)
#Assume NAs in language exposure are zeros

participants_language <- pd %>%
  select(lab, subid, lang_group, session_error, notes, lang1, lang2, lang3, lang4, lang1_exposure, lang2_exposure, lang3_exposure, lang4_exposure) %>%
  ...
  group_by(lab, subid) %>%
  mutate(total_exposure = sum(lang1_exposure, lang2_exposure, lang3_exposure, lang4_exposure))

participants_language_fix <-
  participants_language %>%
  #filter(total_exposure >101 | total_exposure < 99 | is.na(lang_group)) %>%
  filter(total_exposure >101 | total_exposure < 99) %>%
  filter(session_error != "error") %>%
  filter(total_exposure != 200) # These are babies with doubled rows


write_csv(participants_language_fix, "metadata/participants_language_fix.csv")
