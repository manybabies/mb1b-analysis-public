...
Based on `r n_bil_nae` infants, the interaction between IDS and NAE exposure was still statistically significant ($\beta$ = `r round(nae_only_coef$Estimate[13], 3)`, $SE = `r round(nae_only_coef$SE[13], 3)`$, $p = `r round(nae_only_coef$p[13], 3)`)$. This result suggested that a dose-response relationship between infants’ exposure to NAE and their preference for IDS over ADS was not driven by infants living in non-NAE contexts alone (see Table 7 for details of the model).
