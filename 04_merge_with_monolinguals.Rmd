...
```{r}
 ...
 mono_all <- read.csv(text=RCurl::getURI("https://raw.githubusercontent.com/manybabies/mb1-analysis-public/83358f804335452fe50e3eaf2831575635943ec9/processed_data/03_data_trial_main.csv")) 

mono_all <- mono_all %>% 
   mutate(subid = paste(subid, lab, sep = "_")) #fix because subid sometimes repeats across labs
#mono_all <- read.csv(here("mb1-analysis-public/processed_data/03_data_trial_main.csv") # Reading from computer instead
...
mono_all <- mono_all %>%
  ...
  filter(!is.na(mono_all$trial_type)) #Removes all rows with unknown trial type.  Introduced in MB1 script due to missingness analysis
