...
subject_NA_matched <- d_matched %>% 
  filter(subid == setdiff(d_matched$subid, d_lmer_matched$subid)) #finding out who has NA for all LT time, all of them are monolinguals
...
# mod_lmer_matched <- lmer(log_lt ~
  ...
  # (trial_type * age_mo_c * lang_group |lab) +
  # (method + age_mo_c * nae + age_mo * lang_group |item),
#   data = d_lmer_matched)

# The followings were removed sequentially in the order listed below:
# 1. "method" as random slope from the item group level 
# 2. "age_mo_c * nae" as random slope from the item group level 
# 3. "age_mo_c * lang_group" as random slope from the item group level 
# 4. "age_mo_c" as random slope from the item group level 
...
# 10. "trial_type * age_mo_c * lang_group" as random slope from the lab level
# 11. "age_mo_c" as random slope from the lab level
...
mod_lmer_matched_4_interaction <- lmer(log_lt~
  trial_type * method + age_mo_c * trial_num +
  trial_type * trial_num +
  trial_type * age_mo_c * lang_group * nae +
  (1 |subid) +
  (1 |lab), 
  data = d_lmer_matched)
...
subject_NA_full <- d_full %>% 
  filter(subid == setdiff(d_full$subid, d_lmer_unmatched$subid)) #finding out who has NA for all LT time, all of them are monolinguals
...
# nae_lmer_matched <- lmer(log_lt~
  ...
  # (trial_num * trial_type |subid) +
  # (trial_type * age_mo_c * nae_exp |lab) +
  # (method + age_mo_c * nae_exp * trial_num |item),
#   data = d_lmer_matched)

# The followings were removed sequentially in the order listed below:
# 1. "method" as random slope from the item group level 
# 2. "age_mo_c * nae_exp * trial_num" as random slope from the item group level 
# 3. "age_mo_c * trial_num" as random slope from the item group level 
# 4. "age_mo_c * nae_exp" as random slope from the item group level 
# 5. "age_mo_c" as random slope from the item group level 
...
# 11. "trial_type * age_mo_c * nae_exp as random slope from the lab level
# 12. "age_mo_c" as random slope from the lab level
...
#   trial_type * age_mo_c * nae +
#   trial_type * age_mo_c * lang_group +
#   trial_type * age_mo_c * SES +
#   (trial_num * trial_type | subid) +
#   (trial_type * age_mo_c * lang_group | lab) +
#   (method + age_mo_c * lang_group + age_mo_c * nae + age_mo_c * SES | item),
#   data = d_lmer_matched
#   )
...
# 2. "age_mo_c * nae" as random slope from the item group level 
# 3. "age_mo_c * lang_group" as random slope from the item group level 
# 4. "age_mo_c * SES" as random slope from the item group level 
# 5. "age_mo_c" as random slope from the item group level 
...
# 12. "trial_type * age_mo_c* lang_group" as random slope from the lab level
# 13. "age_mo_c" as random slope from the lab level
# 14. "trial_type" as random slope from the lab level
# 15. "lang_group" as random slope from the lab level
# 16. (1 | item) as random intercept from the model
# 17. trial_type * age_mo_c * SES as fixed effect from the model becuase of multicollinearity
# 18. trial_type * SES as fixed effect from the model becuase of multicollinearity
...
# 14. "trial_type" as random slope from the lab level
# 15. "lang_group" as random slope from the lab level
# 16. "trial_type * age_mo_c * SES" as fixed effect because of multicollinearity
