...
################################ nae_ind ################################
## Checking at an individual level, with NAE labs, whether lang exposure was actually 
## English & X, rather than assuming NAE exposure. 

nae_ind <- function(x) {
  df <- NULL
  
  for (i in 1:nrow(NAE_labs)) {
    y <- subset(x, lab == NAE_labs$lab[i])
    
    for (j in 1:nrow(y)) {
      y$nae <- ifelse(str_detect(y$lang1, fixed("English", ignore_case = TRUE)) == TRUE, TRUE,
                      ifelse(str_detect(y$lang2, fixed("English", ignore_case = TRUE)) == TRUE, TRUE,
                             ifelse(str_detect(y$lang3, fixed("English", ignore_case = TRUE)) == TRUE, TRUE,
                                    ifelse(str_detect(y$lang4, fixed("English", ignore_case = TRUE)) == TRUE, TRUE, FALSE))))
    }
    df = rbind(df, y)
  }
  
  x %>% 
    anti_join(NAE_labs, by="lab") %>% 
    bind_rows(df) -> z
  
  z %>%
    mutate(nae=replace(nae, is.na(z$nae), FALSE)) -> a
  
  return(a)
}

################################ comm_lang_learned ################################
## Checking at individual level whether children per lab are learning the community 
## language (TRUE or FALSE) (i.e., FALSE if children are learning Mandarin-English in 
## Paris)

comm_lang_learned <- function(x) {
  df <- NULL
  
  for (i in 1:nrow(lab_community_language)) {
    y <- subset(x, lab == lab_community_language$lab[i])
    
    for (j in 1:nrow(y)) {
      
      y$comm_lang_learn <- ifelse(str_detect(y$lang1, lab_community_language$comm_lang[i]) == TRUE & y$lang1_exposure > 24, TRUE,
                                  ifelse(str_detect(y$lang2, lab_community_language$comm_lang[i]) == TRUE & y$lang2_exposure > 24, TRUE,
                                         ifelse(str_detect(y$lang3, lab_community_language$comm_lang[i]) == TRUE & y$lang3_exposure > 24, TRUE,
                                                ifelse(str_detect(y$lang4, lab_community_language$comm_lang[i]) == TRUE & y$lang4_exposure > 24, TRUE, 
                                                       ifelse(is.na(str_detect(y$lang3, lab_community_language$comm_lang[i])) == TRUE, FALSE, 
                                                              ifelse(is.na(str_detect(y$lang4, lab_community_language$comm_lang[i])) == TRUE, FALSE, FALSE))))))
    }
